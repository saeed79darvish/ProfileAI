...
/**
 * "Continue with Google" button. Opens the ProfilleAI web login/register
 * (Google + email) and syncs the session back into the extension once the
 * user signs in.
 */
export const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({ onAuthSync, hint, mode = 'login' }) => {
  ...
  return (
    <>
      ...
      >
        ...
        {webSyncing ? 'Waiting for sign-in…' : mode === 'register' ? 'Sign up with Google' : 'Continue with Google'}
      </button>
      {webSyncing ? (
