...
const startServer = async () => {
  try {
    ...
    const server = app.listen(PORT, '0.0.0.0', () => {
      ...
      // Corpus-hygiene sweeps (embed/skill backfill, ghost scan, prune,
      // compaction, dedupe, board cap, board refresh). By default the
      // dedicated cronWorker process owns these (see workers/sweepWorker.js) —
      // running them inline here too, on the same dyno that serves requests,
      // is what caused overlapping sweeps to deadlock each other on
      // "ExternalJobs" and pile up enough concurrent memory pressure to crash
      // the process with "JavaScript heap out of memory". Set
      // RUN_SWEEPS_INLINE=true only for local/dev convenience when you're not
      // running `npm run cron` separately.
      if (process.env.RUN_SWEEPS_INLINE === 'true') {
        const { startSweeps } = require('./workers/sweepWorker');
        startSweeps();
      } else {
        console.log('[Sweeps] Inline sweeps disabled. Run `npm run cron` as a separate process.');
      }
